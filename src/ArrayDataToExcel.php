...
class ArrayDataToExcel extends Component
{
    ...
    public function create($saveAsFile=true, $filename=null)
    {
        ...
        if (is_null($filename)) {
            $filename = "{$options[filename_prepend]} {$today}.xlsx";
        }
        ...
        // Save Output
        $writer = IOFactory::createWriter($spreadsheet, $options['writer']);
        $writer->save($output);
    }
